			...
			editor.on('paste', function(e) {
				...
				if (clipboardData) {
					...
					if (shouldPasteAsPlainText()) {
					    ...
					    //SAAR - chrome paste image
					    var processed = false;
					    try {
					        for (var ct = 0; ct < clipboardData.items.length; ct++) {
					            if (clipboardData.items[ct].type == "image/png") {
					                var items = e.clipboardData.items;
					                var blob = items[ct].getAsFile();
					                var reader = new FileReader();
					                reader.onload = function (event) {
					                    processHtml('<img src="' + event.target.result + '"/>')
					                }; // data url!
